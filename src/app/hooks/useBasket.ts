...
const useBasket = () => {
    const cartJson: string | null = localStorage.getItem("cartData");
      const currentCartRaw = cartJson ? JSON.parse(cartJson) : [];
      const currentCart = Array.isArray(currentCartRaw)
      ? currentCartRaw.filter(item => item && typeof item === "object" && !Array.isArray(item))
      : [];
